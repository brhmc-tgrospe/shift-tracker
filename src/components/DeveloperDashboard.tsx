...
import { Briefcase, Users, Trash2, Edit2, Plus, User as UserIcon, VenetianMask, Search, Moon, Sun } from 'lucide-react';
...
export function DeveloperDashboard() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      setUsers(data);
      // Clear selection when users are refetched
  ...
  const handleImpersonate = async (targetUserId: number) => {
    try {
      ...
      // Save current developer token
      if (token) {
        localStorage.setItem('devToken', token);
      }

      // data.token and data.user are returned
      // Calling login will update context and token
  ...
  return (
    <div className="min-h-screen bg-gray-100/50 dark:bg-gray-900 transition-colors">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-indigo-600 dark:bg-indigo-500 rounded-xl flex items-center justify-center shadow-sm text-white">
                <Users className="w-6 h-6" />
              </div>
              <h1 className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">Developer Dashboard</h1>
            </div>
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => navigate('/profile')} className="text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400 transition-colors" title="Profile">
                <UserIcon className="w-5 h-5" />
              </button>
              <button onClick={handleLogout} className="text-sm text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 font-medium">
                Logout
              </button>
            </div>
          </div>
                    ...
                    <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <input 
                          ...
                          disabled={u.id === user?.id} // Don't allow selecting oneself for deletion
                        />
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${u.role === 'Developer' ? 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200' : 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200'}`}>
                          {u.role}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        ...
                        {u.role !== 'Developer' && (
                          <button onClick={() => handleImpersonate(u.id)} className="text-orange-600 hover:text-orange-900 dark:text-orange-400 dark:hover:text-orange-300 mr-4 transition-colors" title="Impersonate User">
                            <VenetianMask className="w-4 h-4 inline" /> <span className="sr-only">Impersonate</span>
                          </button>
                        )}

                        {u.id !== user?.id && (
                          <button onClick={() => handleDelete(u.id)} className="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300 transition-colors">
