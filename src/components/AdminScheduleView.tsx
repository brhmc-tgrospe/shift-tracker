...
import { AutoGenerateScheduleModal } from './AutoGenerateScheduleModal';
...
export function AdminScheduleView() {
  ...
  // Group users by department and filter by department & gender
  ...
  const handlePerformGeneration = async (exemptEveningUserIds: number[]) => {
    ...
    if (existingShifts.length > 0) {
      const confirm = await modal.confirm("There are already plotted schedules for this month. Do you want to generate a new schedule and overwrite them (you must still click Save to finalize)?");
      if (!confirm) return;
    }

    try {
      const generated = await MonthlyScheduleGenerator.generate(
        users,
        currentDate.getFullYear(),
        currentDate.getMonth(),
        { exemptEveningUserIds }
      );
      const newPending = { ...pendingChanges };
      // Clear any prior pending changes for the target month first
      Object.keys(newPending).forEach(key => {
        if (newPending[key]?.date?.startsWith(targetMonthStr)) {
      ...
      toast.success("Schedule generated successfully! Review the plotted shifts and click 'Save Changes' to finalize.");
    } catch (err: any) {
      toast.error(err.message);
    }
  };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 flex flex-col h-full max-h-screen print:p-0 print:m-0 print:h-auto print:max-h-none print:block print:overflow-visible">
      ...
      <AutoGenerateScheduleModal
        ...
        onGenerate={handlePerformGeneration}
        nonItUsers={users.filter(u => u.department_name !== 'IT Regular')}
